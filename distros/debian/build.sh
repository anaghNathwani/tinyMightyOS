#!/usr/bin/env bash
# distros/debian/build.sh — Debian 12 ARM installer for macOS dual-boot

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(cd "${SCRIPT_DIR}/../.." && pwd)"
IMAGE_PATH="${REPO_DIR}/build/debian-arm64.img.gz"
LOG="/tmp/tinymightyos-debian-install.log"
...
download_debian_arm64() {
  if [[ -f "${IMAGE_PATH}" ]]; then
    info "Found existing Debian ARM64 image at ${IMAGE_PATH}"
    return
  fi

  mkdir -p "${REPO_DIR}/build"
  info "Downloading Debian 12 ARM64 image..."
  local url="https://cloud.debian.org/images/cloud/bookworm/daily/latest/debian-bookworm-daily-arm64.raw.gz"
  wget -q --show-progress -O "${IMAGE_PATH}" "${url}" || die "Failed to download Debian ARM64"
  [[ -f "${IMAGE_PATH}" ]] || die "Image file missing after download"
  info "Downloaded Debian ARM64 image"
}
...
set chosenDisk to choose from list diskLines with prompt "Select the internal target disk for Debian. This will erase the selected partition." default items {item 1 of diskLines}
...
set confirmText to "Debian will be written to ${target}. This will erase all data on this partition. Continue?"
...
APPLESCRIPT
  ...
  info "Writing Debian ARM64 image to ${target}..."
...
APPLESCRIPT
  ...
  osascript <<APPLESCRIPT
set successText to "Debian has been written to ${target}.\n\nRestart the Mac and hold the power button to open Startup Options. Select the Debian volume to complete setup."
display dialog successText buttons {"OK"} default button "OK"
APPLESCRIPT

  info "Debian installation complete"
}

main() {
  info "Starting Debian 12 ARM64 installer"
  install_brew_deps
  download_debian_arm64
